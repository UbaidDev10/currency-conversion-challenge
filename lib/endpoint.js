...
async function handleBudgetCurrencyRequest (req, res) {
  try {
    console.log('=== START: handleBudgetCurrencyRequest ===')
    ...
    console.log('Searching for project:', projectName, 'year:', year)
    ...
    res.json(response)
    console.log('=== END: handleBudgetCurrencyRequest ===')
  } catch (error) {
    console.error('Error in budget currency request:', error)
    res.status(500).json({
      success: false,
